...
if __name__ == '__main__':
    ...
    parser.add_argument('--result-path', type=str, default=os.path.join(os.path.dirname(__file__), 'results/'))
    parser.add_argument('--avatar', type=str, default=None, help="avatar to perform inference")
    ...
    if opt.avatar is not None:
        print('Using avatar:', opt.avatar)
        avatar_dir = os.path.join(os.path.dirname(__file__), 'avatars', opt.avatar)
        data_path = os.path.join(avatar_dir, 'data')
        model_path = os.path.join(avatar_dir, 'model')
        opt.path = data_path
        opt.workspace = model_path
        result_id = f'{opt.avatar}_{opt.aud.split(".")[0]}'.replace('/', '_')
        result_path = os.path.join(os.path.dirname(__file__), 'results', result_id)
    ...
    if opt.test:
        ...
        trainer.test(test_loader,save_path=result_path)
